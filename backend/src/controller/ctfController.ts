 ...
 */
const ctfInstancePreparation = (
  workingContainers: containerInstance[],
  userId: string,
  ctfId: string,
  expiresAt: Date,
): ICtfInstance | AppError => {
  ...
  return {
    ...
    url: extractedUrl as string,
    containers: workingContainers.map((c): ICftInstanceContainer => {
      return {
        name: c.name,
        port: c.port,
      };
 ...
 */
function isAppError(obj: ICtfInstance | AppError): obj is AppError {
...
export const submitFlag = async (
  ...
  const userId = (req as reqWithId).userId;
  const solvedCtfs = (req as reqWithId).solvedCtf;
...
const portReservations = new Set<number>();
 ...
 */
export const createCtfInstance = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  ...
  let containersToStopInFailure: string[] = [];
  ...
  let instanceToSetup: ICtfInstanceModel | undefined = undefined;
  let resInstances;
  ...
  if (ctfInstance) {
    ...
    if (ctfInstance.ctfId.equals(ctfContainersConfig._id)) {
      ...
      return next({
        ...
        message:
          'You already have an active instance of another challenge. Please stop it before starting a new one.',
        statusNumber: HTTP_CODE.CONFLICT,
      });
    }
  }
  ...
  const ports: number[] = resInstances.flatMap((instance) =>
    instance.containers.filter((c) => c.port !== undefined).map((c) => c.port!),
  );
  ...
  const containers: containerConfig[] =
    ctfContainersConfig.containersConfig || [];
  ...
  if (containers.length === 2) {
    ...
    const ctfInstanceWithoutDb: containerInstance[] | AppError =
      await startInstanceWithoutDb(
        containers,
        userId.toString(),
        validPort,
        ctfContainersConfig._id.toString(),
        instanceToSetup._id.toString(),
        timeOfexpiration,
      );
    ...
    if ('status' in ctfInstanceWithoutDb && 'message' in ctfInstanceWithoutDb) {
      ...
      await CtfInstance.findByIdAndDelete(instanceToSetup._id);
      return next({
        name: (ctfInstanceWithoutDb as AppError).name,
        message: (ctfInstanceWithoutDb as AppError).message,
        statusNumber: HTTP_CODE.INTERNAL_SERVER_ERROR,
      });
    }

    // Transform container data into DB-ready format
    const ctfInstanceToCreate = ctfInstancePreparation(
      ctfInstanceWithoutDb as containerInstance[],
      userId.toString(),
      ctfContainersConfig._id.toString(),
      new Date(timeOfexpiration),
    );
    ...
    if (isAppError(ctfInstanceToCreate)) {
      ...
      return next({
        name: (ctfInstanceWithoutDb as AppError).name,
        message: (ctfInstanceToCreate as unknown as AppError).message,
        statusNumber: HTTP_CODE.INTERNAL_SERVER_ERROR,
      });
    }

    // Record container names for potential cleanup
    containersToStopInFailure = ctfInstanceToCreate.containers.flatMap(
      (c) => c.name,
    );

    // Update PENDING → RUNNING in DB
    const updatedInstance = await CtfInstance.findOneAndUpdate(
      { _id: instanceToSetup._id },
      {
        $set: {
          url: (ctfInstanceToCreate as ICtfInstance).url,
          status: instanceState.RUNNING,
          expiresAt: timeOfexpiration,
          containers: (ctfInstanceToCreate as ICtfInstance).containers,
        },
      },
      { new: true },
    );
    ...
    return next({
      ...
      message:
        'CTF configuration must contain exactly 2 containers (frontend + backend)',
      statusNumber: HTTP_CODE.INTERNAL_SERVER_ERROR,
    });
  }
};
 ...
 */
export const stopCtfInstance = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    return next({
      name: ERROR_NAME.INTERNAL_ERROR,
      message:
        error instanceof Error ? error.message : 'Failed to stop CTF instance',
      statusNumber: HTTP_CODE.INTERNAL_SERVER_ERROR,
    });
  }
};
 ...
 */
export const containersCleanUp = async (): Promise<void> => {
  try {
    ...
    console.log(
      `[CRON] Found ${expiredInstances.length} expired instances to clean up`,
    );
    ...
    const cleanupResults = await Promise.allSettled(
      expiredInstances.map(async (instance) => {
        try {
          ...
          console.error(
            `[ERROR] [CRON] Failed to cleanup instance ${instance._id}:`,
            error,
          );
          ...
          try {
            ...
            console.error(
              `[ERROR] Failed to mark instance ${instance._id} as FAILED:`,
              saveError,
            );
          }
    ...
    console.log(
      `[CRON] Cleanup complete: ${successful} successful, ${failed} failed`,
    );
  } catch (error) {
