 ...
 */
export const generateValidPorts = (usedPorts, reservedPorts = new Set()) => {
    for (let i = 0; i < 100; i++) {
 ...
 */
export const stopContainers = async (containersId) => {
    ...
    await Promise.all(ids.map(async (id) => {
        try {
            ...
            if (err &&
                ...
                err.statusCode >= 500) {
                const error = err;
                console.error(`[ERROR] Docker daemon error (${error.statusCode}) for ${id.slice(0, 12)}:`, error.message || 'Unknown error');
            }
        }
    }));
};
 ...
 */
const createChallengeContainer = async (config) => {
    try {
 ...
 */
const waitForContainerHealth = async (containerIdOrName, options = {}) => {
 ...
 */
export const startInstanceWithoutDb = async (containers, userId, validPort, ctfId, ctfInstanceId, expiredAt = Date.now() + 60 * 60 * 1000) => {
    ...
    const backendEnvObj = backendContainerConfig.env instanceof Map
    ...
    const startedContainer = await createChallengeContainer(backendContainerConfig);
    ...
    const isBackendContainerWorking = await waitForContainerHealth(startedContainer.id);
    ...
    const envObj = frontendContainerConfig.env instanceof Map
    ...
    const startedContainerFrontend = await createChallengeContainer(frontendContainerConfig);
    // Handle frontend creation failure: stop both containers if needed
    if ('message' in startedContainerFrontend &&
    ...
    const isFrontendWorking = await waitForContainerHealth(startedContainerFrontend.id);
